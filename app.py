...
# --- Changed: shared green color for all Altair charts ---
CHART_COLOR = "#1F77B4"
...
if data_totp_code:
    ...
    with chart_col1:
        with st.container(border=True):
            ...
            if not region_sales.empty:
                chart = alt.Chart(region_sales).mark_bar(color=CHART_COLOR).encode(
    ...
    with chart_col2:
        with st.container(border=True):
            ...
            if not category_sales.empty:
                chart = alt.Chart(category_sales).mark_bar(color=CHART_COLOR).encode(
    ...
    with st.container(border=True):
        ...
        if not daily_sales.empty:
            chart = alt.Chart(daily_sales).mark_line(color=CHART_COLOR, strokeWidth=2).encode(
    ...
    with bottom_col1:
        with st.container(border=True):
            ...
            if not product_sales.empty:
                chart = alt.Chart(product_sales).mark_bar(color=CHART_COLOR).encode(
    ...
    with bottom_col2:
        with st.container(border=True):
            ...
            if not channel_sales.empty:
                chart = alt.Chart(channel_sales).mark_bar(color=CHART_COLOR).encode(
                    x=alt.X("SALES_CHANNEL:N", sort="-y", title="Sales channel"),
                    y=alt.Y("SALES_AMOUNT:Q", title="Sales amount"),
                )
                st.altair_chart(chart, use_container_width=True)
            else:
